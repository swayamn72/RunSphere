...
M2 endpoints are contract-backed and documented through OpenAPI: adult registration/login/refresh/logout, authenticated privacy zones, and resumable activity create/chunk/finalize/status/history/delete. Every activity endpoint requires `Authorization: Bearer <access-token>`; creation additionally requires `Idempotency-Key`. Chunk uploads are bounded to 500 points and 1 MiB of plain JSON. Every `PUT /v1/activities/:activityId/chunks` needs an `X-Chunk-Checksum` SHA-256 of canonical JSON `{sequence,points}`. Canonical JSON recursively sorts object keys and preserves array order; `canonicalJson` and `activityFinalizeChecksumInput` are exported by `@runsphere/contracts` for the mobile coordinator. Gzip is deliberately rejected in M2 rather than decompressed server-side, avoiding a decompression-bomb boundary. Finalize requires the expected count and SHA-256 of the sequence-ordered concatenated chunk checksum values. Same-sequence/same-checksum replay returns `204`; a different payload returns `409`. `GET /v1/activities/:activityId/sync?expectedChunkCount=N` reports authoritative missing sequences before retrying uploads, and finalize retries with the same checksum return the current state idempotently.
